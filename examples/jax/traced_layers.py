...
from lllm.api.abstract import invoke_abstract, make_abstract

if len(sys.argv) > 1:
    xla_flags = sys.argv[1:]
else:
    xla_flags = [
        "--xla_dump_to=dump_traced",
...
def run_step(num_layers: int = 1, batch_size: int = 16, hidden_size=32):
    params = []
    for _ in range(num_layers):
        dot_params = make_abstract(hidden_size, hidden_size, dtype=np.float32)
        bias_params = make_abstract(hidden_size, dtype=np.float32)
        params.append((dot_params, bias_params))
    step = jit(step_fn)
    x = make_abstract(batch_size, hidden_size, dtype=np.float32)
    truth = make_abstract(hidden_size, dtype=np.float32)
    _ = invoke_abstract(step, params, x, truth)
