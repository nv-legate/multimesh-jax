...
ENTRY main.15 {
  ...
  tangent.8 = f32[8]{0} tan(multiply.7), metadata={op_name="jit(c)/jit(main)/task_g/tan"}
  opt-barrier.1 = f32[8]{0} opt-barrier(tangent.8)
...
ENTRY main.15 {
  ...
  multiply.7 = f32[8]{0} multiply(multiply.4, broadcast.6), metadata={op_name="jit(c)/jit(main)/task_f/mul"}
  tangent.8 = f32[8]{0} tan(multiply.7), metadata={op_name="jit(c)/jit(main)/task_g/tan"}
  tuple.1 = (f32[8]{0}, f32[8]{0}) tuple(multiply.7, tangent.8)
  opt-barrier.1 = (f32[8]{0}, f32[8]{0}) opt-barrier(tuple.1)
  gte.tangent.8 = get-tuple-element(opt-barrier.1), index=1
  add.9 = f32[8]{0} add(gte.tangent.8, multiply.7), metadata={op_name="jit(c)/jit(main)/task_g/add"}
  tuple.2 = (f32[8]{0}, f32[8]{0}) tuple(add.9, tangent.8)
  opt-barrier.2 = (f32[8]{0}, f32[8]{0}) opt-barrier(tuple.2)
  gte.add.9 = get-tuple-element(opt-barrier.2), index=0
  ROOT add.10 = f32[] add(gte.tangent.8, gte.add.9)
} // main.15
)";
...
TEST_F(MpmdUnpackOptimizationBarrierTest, TaskWithBarrierTuple) {
  ...
  EXPECT_THAT(module->entry_computation()->instructions(),
              Contains(op::CustomCall(op::Add())));
  EXPECT_THAT(module->entry_computation()->instructions(),
              Contains(op::CustomCall(op::Multiply())));
  EXPECT_THAT(module->entry_computation()->instructions(),
              Contains(op::CustomCall(op::Tan())).Times(2));
}
