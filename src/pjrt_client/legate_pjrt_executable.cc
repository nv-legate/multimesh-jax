...
WrapperPjRtExecutable::Execute(
  ...
  for (const auto& handle_vec : argument_handles) {
    ...
    for (auto* buf : handle_vec) {
      auto* legate_buf = dynamic_cast<LegatePjRtBuffer*>(buf);
      if (legate_buf) {
        if (legate_buf->has_host_action()) {
          TF_RETURN_IF_ERROR(legate_buf->ResolveHostAction());
        }
        if (!legate_buf->has_native_buffer()) {
          return InvalidArgumentStrCat(
              "WrapperPjRtExecutable::Execute: received legate buffer not "
              "backed by native buffer");
        }
        unwrapped_handles.push_back(legate_buf->native_buffer());
      } else {
...
std::vector<std::unique_ptr<PjRtBuffer>> WrapperPjRtExecutable::Wrap(
  ...
  for (auto& buf : outputs) {
    ...
    auto legate_buffer = std::make_unique<LegatePjRtBuffer>(
        ...
        std::move(device_shape), legate_client_, wrapped_->client(), device,
        memory_space, "wrapped");
    wrapped.push_back(std::move(legate_buffer));
    ++output_index;
  }
...
PjRtClient* LegatePjRtExecutable::client() const { return legate_client_; }

int LegatePjRtExecutable::num_replicas() const {
  return device_assignment_->replica_count();
}

int LegatePjRtExecutable::num_partitions() const {
  return device_assignment_->computation_count();
}

int64_t LegatePjRtExecutable::SizeOfGeneratedCodeInBytes() const { return 0; }

const DeviceAssignment& LegatePjRtExecutable::device_assignment() const {
  return *device_assignment_;
}

absl::Span<const PjRtLoadedExecutable::LogicalDeviceIds>
LegatePjRtExecutable::addressable_device_logical_ids() const {
  return addressable_device_logical_ids_;
}

absl::Span<PjRtDevice* const> LegatePjRtExecutable::addressable_devices()
    const {
  return addressable_devices_;
}

LegatePjRtExecutable::LegatePjRtExecutable(
    ...
    : legate_client_(legate_client),
      ...
      context_(legate_client->shared_context()),
      fast_path_exe_(std::move(fast_path_exe)) {}

void LegatePjRtExecutable::Delete() { deleted_ = true; }

bool LegatePjRtExecutable::IsDeleted() { return deleted_; }

absl::StatusOr<std::vector<std::shared_ptr<HloModule>>>
LegatePjRtExecutable::GetHloModules() const {
  return Unimplemented("LegatePjRtExecutable::GetHloModules");
}

absl::StatusOr<std::vector<std::unique_ptr<PjRtBuffer>>>
LegatePjRtExecutable::ExecuteSharded(
    absl::Span<PjRtBuffer* const> argument_handles, PjRtDevice* device,
    const ExecuteOptions& options, std::optional<PjRtFuture<>>& returned_future,
    bool fill_future) {
  LOG(ERROR) << "Unexpected call to LegatePjRtExecutable::ExecuteSharded.";
  return Unimplemented("ExecuteSharded");
}

absl::StatusOr<std::vector<std::unique_ptr<PjRtBuffer>>>
LegatePjRtExecutable::ExecutePortable(
    absl::Span<PjRtBuffer* const> argument_handles, PjRtDevice* device,
    const ExecuteOptions& options, std::optional<PjRtFuture<>>& returned_future,
    bool fill_future) {
  LOG(ERROR) << "Unexpected call to LegatePjRtExecutable::ExecutePortable.";
  return Unimplemented("ExecutePortable");
}

absl::StatusOr<std::vector<std::vector<std::unique_ptr<PjRtBuffer>>>>
LegatePjRtExecutable::Execute(
    absl::Span<const std::vector<PjRtBuffer*>> argument_handles,
    const ExecuteOptions& options,
    std::optional<std::vector<PjRtFuture<>>>& returned_futures) {
  VLOG(1) << "LegatePjRtExecutable::Execute called for '" << name();
  ...
  // for every output shape create fresh output LegateBuffer with Store
  ...
  for (auto&& argument_vec : argument_handles) {
    for (PjRtBuffer* buffer : argument_vec) {
      auto* legate_buffer = dynamic_cast<LegatePjRtBuffer*>(buffer);
      if (!legate_buffer) {
        return InvalidArgumentStrCat(
            "non-legate buffer passed to Legate executable");
      }
      all_native_buffer_inputs =
          all_native_buffer_inputs && legate_buffer->has_native_buffer();
      if (fast_path_exe_ && !legate_buffer->has_native_buffer()) {
        VLOG(3) << legate_buffer->name()
  ...
  if (fast_path_exe_ && all_native_buffer_inputs) {
    ...
    // we don't have any Legate stores to process and the executable is small
    ...
    for (auto&& handles : argument_handles) {
      ...
      for (auto&& input : task.inputs) {
        // inputs may get permuted from the legate compilation process
  ...
  auto legate_stream =
      std::make_shared<LegateStream>(legate_client_->backend());
  ...
  for (size_t local_device = 0; local_device < num_local_devices;
    ...
    for (PjRtBuffer* const input : arguments) {
      auto* legate_buffer = dynamic_cast<LegatePjRtBuffer*>(input);

      if (legate_buffer->has_store()) {
        zuku::ShardedShape sharded_shape =
            context_->GetStoreShardedShape(legate_buffer->store());
        VLOG(3) << "initialized Arg_" << index
                << " with shape=" << sharded_shape;
        parameter_stores[local_device].push_back(legate_buffer->store());
      } else {
        ...
        TF_ASSIGN_OR_RETURN(
            zuku::ShardedShape sharded_shape,
            XlaShapeToLegateShape(legate_buffer->on_device_shape(),
                                  get_sharding_devices(global_sharding),
                                  global_sharding,
                                  /*global_shape*/ false));
        TF_ASSIGN_OR_RETURN(
            auto store, legate_buffer->ToStore(sharded_shape, legate_stream));
  ...
  for (size_t out = 0; out < out_shapes.size(); ++out) {
    ...
    for (size_t comp = 0; comp < num_local_devices; ++comp) {
      ...
      auto legate_buffer = std::make_unique<LegatePjRtBuffer>(
          ...
          output_names[out]);
      if (!legate_buffer->on_device_shape().has_layout()) {
      ...
      if (legate_buffer->store().impl == nullptr) {
        return InternalStrCat("output ", out, " for executable ", name_,
                              " got a null Legate store");
      }
      results[comp].push_back(std::move(legate_buffer));
    }
  }
...
LegatePjRtExecutable::GetParameterLayouts() const {
...
absl::StatusOr<std::vector<std::shared_ptr<const PjRtLayout>>>
LegatePjRtExecutable::GetOutputLayouts() const {
  std::vector<std::shared_ptr<const PjRtLayout>> layouts;
...
LegatePjRtExecutable::GetOutputMemoryKinds() const {
