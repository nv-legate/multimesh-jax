...
#ifndef XLA_PJRT_MULTIMESH_TEST_BASE_H_
#define XLA_PJRT_MULTIMESH_TEST_BASE_H_
...
class MultiMeshTestBase : public HloTestBase {
 public:
  struct Config {
  ...
  absl::StatusOr<std::unique_ptr<MultiMeshPjRtExecutable>> Compile(
      absl::string_view hlo_string, int num_devices, Config cfg);

 protected:
  void MultiMeshMockReset();
  ...
  int64_t MultiMeshMockStateHash() const;

  void MultiMeshMockSetComputeHashes(bool flag);
  ...
  static std::unique_ptr<MultiMeshClient> client_;
