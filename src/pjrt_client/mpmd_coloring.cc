...
absl::StatusOr<bool> MpmdColoring::PropagateLoopColorDepth(
  ...
  if (while_computation == nullptr) {
    return absl::InvalidArgumentError("No while loop found in the computation");
  }
...
absl::StatusOr<bool> MpmdColoring::Run(
  ...
  if (enforce_bijective_tasks) {
    TF_ASSIGN_OR_RETURN(
        bool propagated_bijective,
        PropagateLoopColorDepth(module->entry_computation(), properties,
