...
se::Platform* LegateTestBase::platform_{nullptr};
std::unique_ptr<Backend> LegateTestBase::backend_{nullptr};
std::unique_ptr<LegateClient> LegateTestBase::client_{nullptr};
static constexpr int kMaxNumDevices = 128;

absl::StatusOr<HloSharding> LegateTestBase::GetSharding(
...
void LegateTestBase::SetUp() { LegateMockReset(); }

void LegateTestBase::SetUpTestSuite() {
  ...
  client_ = std::make_unique<LegateClient>(
      *std::move(native_client), backend_.get(),
      std::make_shared<MockZukuExecuteContext>());
}

void LegateTestBase::TearDownTestSuite() {
  ClearCachedStreams();
  client_ = nullptr;
  platform_ = nullptr;
  backend_ = nullptr;
...
void LegateTestBase::TearDown() {
  LegateMockReset();
  HloTestBase::TearDown();
  ClearMetadataNameTasks();
}

MockZukuExecuteContext* LegateTestBase::context() const {
  return dynamic_cast<MockZukuExecuteContext*>(client_->mutable_context());
}

void LegateTestBase::LegateMockReset() { context()->Reset(); }

int64_t LegateTestBase::DeviceBytesHighWatermark(
    int64_t local_device_id) const {
  return context()->DeviceBytesHighWatermark(local_device_id);
}

int64_t LegateTestBase::HostBytesHighWatermark(int64_t local_device_id) const {
  return context()->HostBytesHighWatermark(local_device_id);
}

void LegateTestBase::SetCompileModules(bool flag) {
  context()->SetCompileModules(flag);
}

int64_t LegateTestBase::LegateMockStateHash() const {
  return context()->OperationHash();
}

void LegateTestBase::LegateMockSetComputeHashes(bool flag) {
  context()->SetComputeHashes(flag);
}

DeviceAssignment LegateTestBase::GetDeviceAssignment(int num_devices) {
...
bool LegateTestBase::Skip(int num_devices) const {
  return client_->device_count() < num_devices;
}

absl::StatusOr<std::string> LegateTestBase::GetFileText(
...
absl::StatusOr<std::unique_ptr<LegatePjRtExecutable>> LegateTestBase::Compile(
  ...
  auto* legate_exe = dynamic_cast<LegatePjRtExecutable*>(exe.release());
  if (!legate_exe) {
    return InvalidArgumentStrCat(
        "produced executable that is not a LegatePjRtExecutable");
  }
  return std::unique_ptr<LegatePjRtExecutable>(legate_exe);
}
}  // namespace xla