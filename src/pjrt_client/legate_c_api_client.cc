...
constexpr char kPluginName[] = "legate";
...
absl::StatusOr<std::unique_ptr<PjRtClient>> GetTfrtPluginDeviceClient(
  ...
  if (is_gpu) {
    ...
    return std::unique_ptr<PjRtClient>(std::make_unique<LegateClient>(
...
PJRT_Error* PJRT_CpuDeviceTopology_Create(
  ...
  return new PJRT_Error{tsl::errors::Unimplemented(
      "Topology not supported for Legate compilation.")};
}
...
PJRT_Error* PJRT_Legate_Register_Custom_Call(
...
extern "C" const PJRT_Api* GetLegatePjrtApi() {
  static PJRT_Gpu_Custom_Call custom_call{
      ...
      /*next=*/
      reinterpret_cast<PJRT_Extension_Base*>(
          &legate_plugin::custom_partitioner),
      /*custom_call=*/legate_plugin::PJRT_Legate_Register_Custom_Call,
  };
  ...
  static const PJRT_Api pjrt_api = pjrt::CreatePjrtApi(
      legate_plugin::PJRT_Client_Create,
      legate_plugin::PJRT_ExecuteContext_Create,
      legate_plugin::PJRT_CpuDeviceTopology_Create, legate_plugin::PJRT_Init,
...
XLA_REGISTER_CUSTOM_CALL_TARGET_WITH_SYM("Marking", NoOpCustomCall,
                                         xla::kCudaPluginName);
XLA_REGISTER_CUSTOM_CALL_TARGET_WITH_SYM("LegateTask", NoOpCustomCall,
                                         xla::kPluginName);
XLA_REGISTER_CUSTOM_CALL_TARGET_WITH_SYM("LegateTask", NoOpCustomCall,
                                         xla::kCudaPluginName);
