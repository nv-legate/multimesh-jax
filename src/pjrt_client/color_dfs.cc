  ...
  while (!to_visit.empty()) {
    ...
    if (!visited.contains(next)) {
      ...
      std::stable_sort(operands.begin(), operands.end(),
                       [&](HloInstruction* lhs, HloInstruction* rhs) {
                         ...
                         if (lhs_color.has_value() && rhs_color.has_value()) {
                           // higher priority means first in the operand list
                           // (and later in the DFS)
                           return color_priority[*lhs_color] >
                                  color_priority[*rhs_color];
                         }
                         // anything with a color should be moved first in the
                         // operands (and later in the DFS)
      ...
      for (auto* operand : operands) {
        VLOG(5) << "   color=" << Color(operand).value_or("none") << " "
                << operand->name() << " is next operand";
    ...
    if (next->opcode() == HloOpcode::kGetTupleElement &&
        next->operand(0)->opcode() == HloOpcode::kCustomCall) {
