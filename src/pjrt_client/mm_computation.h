...
#ifndef XLA_PJRT_MULTIMESH_MM_COMPILATION_H_
#define XLA_PJRT_MULTIMESH_MM_COMPILATION_H_
...
class MultiMeshXla {
 public:
  static absl::StatusOr<std::unique_ptr<MultiMeshXla>> Create(
      std::unique_ptr<HloModule> module, Shape root_shape, Backend* backend,
      PjRtClient* client);

  ~MultiMeshXla() = default;

  absl::Status Compile(uint64_t run_id, const MultiMeshCompileConfig& config);
 ...
 private:
  ...
  MultiMeshXla(std::unique_ptr<HloModule> module, Backend* backend,
...
class MultiMeshStream {
 public:
  explicit MultiMeshStream(Backend* backend) : backend_(backend) {}

  ~MultiMeshStream() = default;
...
class MultiMeshExecutable {
 ...
 private:
  std::shared_ptr<MultiMeshXla> computation_;
};

class MultiMeshCompiler : public MultiMeshStream {
 public:
  static absl::StatusOr<std::unique_ptr<MultiMeshCompiler>> Create(
      std::unique_ptr<HloModule> module, Shape root_shape, Backend* backend,
      PjRtClient* client);

  ~MultiMeshCompiler() = default;
  ...
  void Compile(uint64_t run_id, const MultiMeshCompileConfig& config);
  ...
  std::unique_ptr<MultiMeshExecutable> MakeExecutable();
 ...
 private:
  explicit MultiMeshCompiler(std::unique_ptr<MultiMeshXla> comp);
...
#endif  // XLA_PJRT_MULTIMESH_MM_COMPILATION_H_
