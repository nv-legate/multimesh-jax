...
class MpmdMicrobatchLoopCanonicalizerTest : public MpmdTestBase {};
...
TEST_F(MpmdMicrobatchLoopCanonicalizerTest, BasicLoop) {
  TF_ASSERT_OK_AND_ASSIGN(
      auto module,
      GetHloModuleFromText(kMultipleMicrobatchSlicesHlo, /*num_devies=*/1));

  CallInliner call_inliner{};
  TF_ASSERT_OK_AND_ASSIGN(bool changed, call_inliner.Run(module.get()));

  MpmdMicrobatchLoopCanonicalizer loop_inliner{partition_.get()};
  TF_ASSERT_OK_AND_ASSIGN(changed, loop_inliner.Run(module.get()));

  // all of the microbatch custom calls should have been removed
  EXPECT_THAT(
