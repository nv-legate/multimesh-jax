...
void MockZukuExecuteContext::CreateCompileTask(
  ...
  xla::LegateCompiler* xla_compiler =
      dynamic_cast<xla::LegateCompiler*>(compiler.get());
  if (compute_hashes_) {
  ...
  if (compile_modules_ && local_device_id == 0) {
    LegateCompileConfig config{/*replica_count=*/1,
...
void MockZukuExecuteContext::CreateExecuteTask(
    ...
    zuku::DeviceList mesh, std::shared_ptr<LegateCompiler> compiler,
    const std::vector<ScalarArgument>& scalars,
    const std::vector<StoreHandle>& inputs,
    const std::vector<StoreHandle>& outputs,
    zuku::Future<zuku::ArrayTile>& temp, LegateExecuteOptions options) {
