...
class MultiMeshClientTest : public MultiMeshTestBase {
 public:
  MultiMeshClientTest() : MultiMeshTestBase() {}
...
TEST_F(MultiMeshClientTest, ArgumentReshard) {
  int num_devices = 4;
  if (Skip(num_devices)) {
  ...
  // shape, MultiMesh doesn't actually care what devices the roots land on
...
TEST_F(MultiMeshClientTest, AutoShardingReshardedParameter) {
