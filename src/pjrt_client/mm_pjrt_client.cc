...
absl::StatusOr<CompileOutput> CreateTasks(
  ...
  auto collect_shapes_and_shardings =
    ...
    VLOG(3) << "MultiMeshClient::Compile: partition "
  ...
  absl::flat_hash_map<HloModule*, std::shared_ptr<MultiMeshCompiler>>
      module_to_compiler;
  for (auto& module : modules) {
    Shape spmd_root_shape =
        GetSpmdShape(module->module->entry_computation()->root_instruction());
    TF_ASSIGN_OR_RETURN(auto compiler, MultiMeshCompiler::Create(
                                           module->module->Clone(""),
...
absl::StatusOr<PjRtDevice*> MultiMeshClient::LookupDevice(
    PjRtGlobalDeviceId device_id) const {
  return base_client_->LookupDevice(device_id);
}

absl::StatusOr<PjRtDevice*> MultiMeshClient::LookupAddressableDevice(
    PjRtLocalDeviceId local_hardware_id) const {
  return base_client_->LookupAddressableDevice(local_hardware_id);
}

absl::string_view MultiMeshClient::platform_version() const {
#define STRINGIFY2(X) #X
#define STRINGIFY(X) STRINGIFY2(X)
  return "multimesh" STRINGIFY(CUDART_VERSION);
}

absl::StatusOr<DeviceAssignment> MultiMeshClient::GetDefaultDeviceAssignment(
    int num_replicas, int num_partitions) const {
  return base_client_->GetDefaultDeviceAssignment(num_replicas, num_partitions);
}

absl::StatusOr<Layout> MultiMeshClient::GetDefaultLayout(
    PrimitiveType element_type, absl::Span<const int64_t> dims) {
  return base_client_->GetDefaultLayout(element_type, dims);
}

absl::StatusOr<std::unique_ptr<PjRtBuffer>>
MultiMeshClient::CreateUninitializedBuffer(const Shape& shape,
...
MultiMeshClient::CreateBuffersForAsyncHostToDevice(
...
absl::StatusOr<std::unique_ptr<PjRtBuffer>>
MultiMeshClient::BufferFromHostBuffer(
    const void* data, PrimitiveType type, absl::Span<int64_t const> dims,
    std::optional<absl::Span<int64_t const>> byte_strides,
    MultiMeshClient::HostBufferSemantics host_buffer_semantics,
    absl::AnyInvocable<void() &&> on_done_with_host_buffer,
    PjRtMemorySpace* memory, const Layout* device_layout) {
  VLOG(3) << "MultiMeshClient::BufferFromHostBuffer for memory "
  ...
  auto buf = std::unique_ptr<PjRtBuffer>(std::make_unique<MultiMeshPjRtBuffer>(
...
absl::StatusOr<std::unique_ptr<PjRtBuffer>>
MultiMeshClient::BufferFromHostLiteral(const LiteralSlice& literal,
                                       PjRtMemorySpace* memory_space) {
...
absl::StatusOr<std::unique_ptr<PjRtBuffer>>
MultiMeshClient::CreateViewOfDeviceBuffer(
...
MultiMeshClient::MakeCrossHostReceiveBuffers(
...
HloModuleProto MultiMeshClient::ShardBatch(const HloModuleProto& proto) const {
...
absl::StatusOr<std::unique_ptr<PjRtLoadedExecutable>> MultiMeshClient::Compile(
  ...
  VLOG(3) << "MultiMeshClient::Compile module for '" << computation.name()
  ...
  if (device_assignment) {
    VLOG(3) << computation.proto().name() << " has device assignment "
            ...
            << " passed into MultiMeshClient::Compile" << std::endl;
    ...
    if (*max_per_process < addressable_device_count() &&
      ...
      return InvalidArgumentStrCat(
          "MultiMeshClient::Compile: cannot create executable on a subset of "
  ...
  if (enable_fast_path_exe && compile_result.schedule.size() == 1 &&
      ...
      !ContainsMultiMeshCustomCall(computation.proto())) {
    VLOG(3) << "MultiMeshClient::Compile: fast path taken for "
            << computation.name();
...
MultiMeshClient::~MultiMeshClient() {
  // The streams must be cleared after shutting down Zuku
