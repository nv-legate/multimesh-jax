...
absl::StatusOr<bool> MpmdReorderShardMapTranspose::VisitLoop(
  ...
  for (auto* instruction : body->MakeInstructionPostOrder()) {
    ...
    // create the new transpose instruction (appropriate, shape, sharding, and color)
    const Shape& dot_shape = dot_instr->shape();
    absl::Span<const int64_t> tr_dims = tr_instr->dimensions();
    TF_ASSIGN_OR_RETURN(Shape new_tr_shape,
    ...
    HloInstruction* new_shard_map_instr = body->AddInstruction(
        HloInstruction::CreateCustomCall(tr_instr->shape(), {new_tr_instr},
        "SPMDShardToFullShape"));
    if (!tr_instr->has_sharding()) {
      InvalidArgumentStrCat("instruction ", tr_instr->name(),
