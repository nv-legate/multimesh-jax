...
void LegateXlaComputation::CostAnalysis(int vlog) {
...
absl::StatusOr<std::shared_ptr<HloModule>> LegateXlaComputation::GetHloModule()
    const {
  if (!executable_) {
    return tsl::errors::InvalidArgument("Legate executable is null");
  }
...
std::optional<int64_t> LegateXlaComputation::OutputAlias(
    int64_t root_index) const {
...
absl::Status LegateXlaComputation::SetupOutputInputAlias() {
...
absl::StatusOr<std::unique_ptr<LegateXlaComputation>>
LegateXlaComputation::Create(std::unique_ptr<HloModule> module,
                             Shape root_shape, Backend* backend,
                             PjRtClient* client) {
  ...
  auto computation = std::unique_ptr<LegateXlaComputation>(
      new LegateXlaComputation(std::move(module), backend, client, launch_size,
                               std::move(root_shape)));
  TF_RETURN_IF_ERROR(computation->SetupOutputInputAlias());
  return std::move(computation);
}

const HloModule& LegateXlaComputation::optimized_module() const {
  if (executable_) {
...
absl::Status LegateXlaComputation::Compile(uint64_t run_id,
                                           const LegateCompileConfig& config) {
...
LegateCompiler::LegateCompiler(std::unique_ptr<LegateXlaComputation> comp)
    : LegateStream(comp->mutable_backend()), computation_(std::move(comp)) {}

absl::StatusOr<std::unique_ptr<LegateCompiler>> LegateCompiler::Create(
    std::unique_ptr<HloModule> module, Shape root_shape, Backend* backend,
    PjRtClient* client) {
  TF_ASSIGN_OR_RETURN(auto comp, LegateXlaComputation::Create(
                                     std::move(module), std::move(root_shape),
                                     backend, client));
  return std::unique_ptr<LegateCompiler>(new LegateCompiler(std::move(comp)));
}

void LegateCompiler::Compile(uint64_t run_id,
                             const LegateCompileConfig& config) {
  absl::Status status;
  try {
  ...
  if (!status.ok()) {
    LOG(ERROR) << "LegateCompiler::Compile of " << Name()
...
std::unique_ptr<LegateExecutable> LegateCompiler::MakeExecutable() {
  Block();
  return std::make_unique<LegateExecutable>(computation_);
}

void LegateCompiler::Block() {
  if (!compilation_finished_) {
    ...
    while (!condition_.wait_for(lock, 1000ms,
      ...
      LOG(ERROR) << "LegateCompiler::Block: "
...
std::optional<std::string> LegateStream::MemcpyHtoDAsync(
...
std::optional<std::string> LegateStream::MemcpyDtoDAsync(
...
absl::Status RunCpuExecutable(uint64_t run_id, cpu::CpuExecutable* exe,
                              xla::Backend* backend,
                              const std::vector<se::DeviceMemoryBase>& inputs,
                              const std::vector<se::DeviceMemoryBase>& outputs,
                              TaskMemoryAllocator* allocator,
                              const LegateDeviceAssignment& device_assignment,
                              bool tupled_args, PjRtClient* client) {
  ...
  for (const BufferAllocation& alloc : exe->buffer_assignment().Allocations()) {
    ...
    if (alloc.is_entry_computation_parameter()) {
      // zero-size allocations get a non-zero size in Legate
      if (alloc.size() != 0 &&
        ...
        return InvalidArgumentStrCat(
            ...
            ", but Legate has ", inputs[alloc.parameter_number()].size());
      }
      ...
      if (alloc.is_tuple()) {
        ...
        if (alloc.size() != 0 && outputs[output_index].size() != alloc.size()) {
          ...
          return InvalidArgumentStrCat(
              ...
              alloc.size(), ", but Legate has ", outputs[output_index].size());
        }
  ...
  TF_ASSIGN_OR_RETURN(DeviceAssignment xla_device_assignment,
                      LegateToXlaDeviceAssignment(device_assignment));
...
absl::Status RunGpuExecutable(zuku::Stream* zs, uint64_t run_id,
                              gpu::GpuExecutable* exe, xla::Backend* backend,
                              const std::vector<se::DeviceMemoryBase>& inputs,
                              const std::vector<se::DeviceMemoryBase>& outputs,
                              TaskMemoryAllocator* allocator,
                              const LegateDeviceAssignment& device_assignment,
                              int64_t num_local, bool tupled_args,
                              bool blocking) {
    ...
    if (alloc.is_thread_local()) {
      ...
      if (tupled_args && alloc.param_shape_index().empty()) {
        ...
        if (inputs[input_index].size() != alloc.size() && alloc.size() > 0) {
          return tsl::errors::InvalidArgument(
              absl::StrCat("Mismatched size on input ", input_index,
                           ": XLA expected ", alloc.size(), " but Legate has ",
...
LegateExecutable::LegateExecutable(
    const std::shared_ptr<LegateXlaComputation>& computation)
    : computation_(computation) {}

std::optional<std::string> LegateExecutable::Execute(
...
std::optional<std::string> LegateExecutable::MemcpyHtoDAsync(
    void* dst, const void* src, size_t size, int64_t local_device_id) const {
  return _MemcpyHtoDAsync(computation_->mutable_backend(), dst, src, size,
                          local_device_id);
}

std::optional<std::string> LegateExecutable::MemcpyDtoDAsync(
    void* dst, const void* src, size_t size, bool cpu,
    int64_t local_device_id) const {
