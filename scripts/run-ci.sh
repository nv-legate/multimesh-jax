...
maxtext_scripts_path=${4:-/opt/workspace/maxtext-scripts}

CUDA_VISIBLE_DEVICES=0 ctest --test-dir $build_dir --parallel $nproc

$SCRIPT_DIR/run-py-tests.sh

pushd $maxtext_scripts_path
$maxtext_scripts_path/validate-cpu.sh
$maxtext_scripts_path/validate-gpu.sh
popd
