...
class MicrobatchTest(LegateJaxTestCase):
    ...
    def test_host_offload_tp2(self):
        if jax.device_count() != 2:
            self.skipTest("need 2 device")

        def layer(param, x):
            x = jnp.einsum("hd,bd->bh", param, x)
            x = jnp.einsum("hd,bd->bh", param, x)
            return x
        ...
        def c(params, x):
            def f(params, x):
                for i in range(num_layers):
                    x = task(layer, mesh=mesh, name=f"layer_{i}")(params[i], x)
                return (x * x).sum()
        ...
        def args_maker():
            params = [
                jnp.arange(dim * dim, dtype=np.float32).reshape(dim, dim)
                / dim
                / dim
        ...
        batch_sharding = jax.sharding.NamedSharding(mesh, P("x", None))
        shardings = (param_shardings, batch_sharding)

        with enable_task_fusion(False):
            with host_offload_min_reuse_distance(2):
                with host_offload_min_size(dim):
                    self._test_against_reference(
                        c,
                        args_maker,
                        arg_shardings=shardings,
                        reference_shardings=shardings,
                    )

    def test_host_offload_pp2(self):
        if jax.device_count() != 2:
            self.skipTest("need 2 device")

        def layer(params, x):
            p0, p1 = params
            x = jnp.einsum("hd,bd->bh", p0, x)
            x = jnp.cos(x)
            x = jnp.einsum("hd,bd->bh", p1, x)
            return x
        ...
        def c(params, batch):
            ...
            f = jax.checkpoint(
                value_and_grad(f), policy=jax.checkpoint_policies.dots_saveable
            )
            f = microbatch(f, argnum=1, dim=0, size=2)
        ...
        def make_tensor():
            return jnp.abs(
                jnp.cos(jnp.arange(size, dtype=np.float32).reshape(dim, dim))
            )

        def init_params():
            return [(make_tensor(), make_tensor()) for _ in range(num_layers)]
        ...
        compiled, _, _ = parallelize(
            c, init_params=init_params, get_input_batch=init_batch
        )
